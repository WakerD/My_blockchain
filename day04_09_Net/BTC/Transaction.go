...
func (tx *Transaction) Sign(privKey ecdsa.PrivateKey, prevTXs map[string]*Transaction) {
	...
	for index, input := range txCopy.Vins {
		...
		//为txCopy设置新的交易ID：txID->[]byte{},Vout,sign-->nil,publicKey-->对应输出的公钥hash
		input.Signature = nil                                 //双保险
		input.PublicKey = prevTx.Vouts[input.Vout].PubKeyHash //设置input的公钥为对应输出的公钥hash
		data := txCopy.getData()                              //设置新的txID

		input.PublicKey = nil //再将publicKey设置为nil

		//txCopy:txID:新id，Vout，sign-->nil，publicKey-->nil
...
func (tx *Transaction) Verify(prevTXs map[string]*Transaction) bool {
	...
	for index, input := range tx.Vins {
		...
		//我们之前为了存储将他们连接在一起，现在我们需要对它们进行解包在 crypto/ecdsa函数中使用

		//验证
		//在这里：我们使用从输入提取的公钥创建了一个 ecdsa.PublicKey,通过传入输入中提取的签名执行了 ecdsa.Verify
		//如果所欲的输入都被验证，返回 ture；如果有任何一个验证失败，返回 false。
