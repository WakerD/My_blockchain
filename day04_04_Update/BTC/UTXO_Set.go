...
//添加一个方法：用于查询给定地址下的，要转账使用的，可以使用的utxo
	...
	err := utxoSet.BlockChain.DB.Update(func(tx *bolt.Tx) error {
		...
		if b != nil {
			...
			for i := 0; i < len(inputs); i++ {
				...
				//根据IxID，如果该txOutputs中已经有output被新区块花掉了，那么将未花掉的添加到utxos里，并标记该txoutputs要删除
				//判断是否需要
				isNeedDelete := false
				utxos := []*UTXO{} //存储未花费
				for _, utxo := range txOutputs.UTXOS {
